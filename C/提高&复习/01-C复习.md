...
void copyStudent(Student* to, Student* from) {
	*to = *from;
}
int main() {
	struct Teacher t; //告诉c编译器给我分配内存
	Student s; // Student用typedef重定义，才能省略struct关键字

	// 初始化变量的三种方法
	Student s1 = { "aaa", 31, 1 };
	// . 是寻址操作：计算age相对于s1大变量的偏移量 ==> 计算 cpu 中进行，没有操作内存
	s1.age = 32;
	strcpy(s1.name, "bbb");
	printf("s1.name:%s \n", s1.name);
	// 通过指针的方式，操作内存空间
...
int createTeacher01 (Teacher** p, int num) {
	Teacher* tmp = NULL;
	tmp = (Teacher*)malloc(sizeof(Teacher) * num);
	if (tmp == NULL) {
		return -1;
	}
	*p = tmp; // 二级指针形参区间接修改实参的值
	return 0;
}
...
static void printTeacher(Teacher* array, int num) {
	int i = 0;
	for (i = 0; i < num; i++) {
		printf("name:%s\talias:%s\tage:%d\n", array[i].name, array[i].alias, array[i].age);
		for (int j = 0; j < 2; j++) {
			printf("student:%s\t", array[i].students[j]);
		}
		printf("\n");
	}
}

static void sortTeacher(Teacher* array, int num) {
	int i, j = 0;
	Teacher tmp;
	for (i = 0; i < num; i++) {
		for (j = i + 1; j < num; j++) {
...
static int createTeacher(Teacher** p, int num) {
	Teacher* tmp = NULL;
	tmp = (Teacher*)malloc(sizeof(Teacher) * num);
	if (tmp == NULL) {
		return -1;
...
static void freeTeacher(Teacher* p, int num) {
	if (p == NULL) {
		return;
	}
	for (int i = 0; i < num; i++) {
...
void main() {
	int i = 0, ret = 0;
	Teacher* p;
	ret = createTeacher(&p, 3);
	if (ret != 0) {
		printf("func createTeacher error");
		return;
	}
	for (i = 0; i < 3; i++) {
		printf("please enter age:");
...
// 编译器的=号操作，只会把指针变量的值从from copy到to,但不会把指针变量所指的内存空间copy过去 //浅copy
void copyTeacher(Teacher* to, Teacher* from) {
	*to = *from;
	to->p_name = (char*)malloc(100);
	strcpy(to->p_name, from->p_name);
...
void main() {
	...
	t1.p_name = (char*)malloc(100);
	strcpy(t1.p_name, "hello");
	copyTeacher(&t2, &t1);
	if (t1.p_name != NULL) {
		free(t1.p_name);
		t1.p_name = NULL;
...
//一旦结构体定义下来，则结构体中的成员内存布局就定下来了
//可以通过age地址去求大的结构体的内存地址
typedef struct Teacher {
	char name[61];
	int age;
	char* p_name;
...
void f_getc() {
	FILE* fp = NULL;
	char* filename1 = "D:/1.txt";
	fp = fopen(filename1, "r+"); //以读写的方式打开文件
	if (fp == NULL) {
		printf("func fopen() err\n");
	}
	else {
...
void f_read() {
	FILE* fp = NULL;
	char* filename1 = "D:/1.data";
	fp = fopen(filename1, "rb"); //以读写的方式打开文件
	if (fp == NULL) {
		printf("func fopen() err\n");
	}
	else {
		...
		Teacher array[3];
		for (int i = 0; i < 3; i++) {
			fread(&array[i], sizeof(Teacher), 1, fp);
			printf("name:%s\n", array[i].name);
			printf("age:%d\n", array[i].age);
		}
	}
