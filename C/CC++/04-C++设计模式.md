...
```
## 中介者模式
- 适用于：用一个中介对象，封装一个对象的交换。中介者是将各个对象不需要显示的相互作用，从而实现了耦合松散，而且可以独立的改变他们之间的交换
```c++
#include <iostream>
using namespace std;
...
class Person {
public:
	Person(string name, int gender, int condition, Mediator* mediator) {
		this->name = name;
		this->gender = gender;
...
public:
	void getParter() {
		if (man->getGender() == woman->getGender()) {
			return;
		}
		if (man->getCondition() == woman->getCondition()) {
			cout << man->getName() << " with " << woman->getName() << " match well" << endl;
		}
		else {
			cout << man->getName() << " with " << woman->getName() << " not match" << endl;
		}
	}
...
public:
	Woman(string name, int gender, int condition, Mediator* mediator) : Person(name, gender, condition, mediator) {
...
public:
	Man(string name, int gender, int condition, Mediator* mediator) : Person(name, gender, condition, mediator) {
...
int main() {
	...
	Person* female = new Woman("小芳", 0, 5, mediator);
	Person* male1 = new Man("张三", 1, 5, mediator);
	Person* male2 = new Man("李四", 1, 4, mediator);
	female->getParter(male1);
	female->getParter(male2);
	cin.get();
	return 0;
}
```
...
public:
	...
	void setPlayerObserver(PlayerObserver* observer) {
...
int main() {
	...
	PlayerObserver* observer1 = NULL;
	PlayerObserver* observer2 = NULL;
	secretary = new Secretary;
	observer1 = new PlayerObserver(secretary);
	observer2 = new PlayerObserver(secretary);
	secretary->setPlayerObserver(observer1);
	secretary->setPlayerObserver(observer2);
	secretary->notify("boss comming");
	secretary->notify("boss going");
	delete observer1;
	delete observer2;
	delete secretary;
	cin.get();
...
```
## 备忘录模式
- 适用于在不破坏封装性的前提下，捕获一个对象的内部状态，并在该对象之外保持这个状态，这样就可以将以后的对象状态恢复到先前保存的状态
- 适用于功能比较复杂的，但需要记录或维护属性历史的类；或者需要保存的属性只是众多属性中的一小部分时Originator(原生者)可以根据保存的Memento(备忘录)还原到前一状态
```c++
#include <iostream>
using namespace std;
//备忘录
...
void memento_test2() {
	...
	person->print();
	caretaker = new Caretaker(person->createMemento());
	person->setAge(33);
	person->print();
	cout << "还原旧的状态" << endl;
...
```
## 访问者模式
- 分离对象的数据和行为，使用visitor模式，可以不修改已有类的情况下，增加新的操作角色和职责
...
class Visitor {
public:
	virtual void visit(ParkElement* e) = 0;
};
class ParkElement {
public:
	ParkElement(string name) {
		this->name = name;
	}
	string getName() {
		return name;
...
public:
	ParkA(string name) : ParkElement(name) {}
	virtual void accept(Visitor* visitor) {
		visitor->visit(this);//公园接受访问者访问，让访问者做操作
	}
};
class ParkB : public ParkElement {
public:
	ParkB(string name) : ParkElement(name) {}
	virtual void accept(Visitor* visitor) {
		visitor->visit(this);//公园接受访问者访问，让访问者做操作
	}
};
...
public:
	Park(string name) : ParkElement(name) {
		elements.clear();
	}
...
private:
	list<ParkElement*> elements;
};
class VisitorA : public Visitor {
public:
	virtual void visit(ParkElement* e) {
		cout << "清洁工A完成公园" << e->getName() << "部分的打扫" << endl;
	}
};
class VisitorB : public Visitor {
public:
	virtual void visit(ParkElement* e) {
		cout << "清洁工B完成公园"<< e->getName() <<"部分的打扫" << endl;
	}
};
class Manager : public Visitor {
public:
	virtual void visit(ParkElement* element) {
		cout << "管理者访问公园的"<< element->getName() <<"部分" << endl;
	}
};
void visitor_test1() {
	Visitor* vA = new VisitorA;
	Visitor* vB = new VisitorB;
	ParkElement* pA = new ParkA("A");
	ParkElement* pB = new ParkA("B");
	pA->accept(vA);
	pB->accept(vB);
	delete pB;
	delete pA;
...
class State {
public:
	virtual void doThing(Worker* w) = 0;
};
class Worker {
public:
	Worker();
...
void State2::doThing(Worker* worker) {
	if (worker->getHour() == 9 || worker->getHour() == 10) {
		...
		cout << "当前时间点：" << worker->getHour() << "未知状态" << endl;
	}
}
Worker::Worker() {
	state = new State1;
...
int main() {
	...
	Expression* expression1 = new PlusExpression(context);
	Expression* expression2 = new MinusExpression(context);
