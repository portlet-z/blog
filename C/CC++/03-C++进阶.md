...
void show_test6() {
	...
	s1.insert("aaa");
	s1.insert("ccc");
	set<string>::iterator it = s1.find("aaa"); //find默认区分大小写
	if (it != s1.end()) {
		cout << "find aaa " << endl;
	}
	else {
...
void pre_test1() {
	...
	cout << endl;
	//求ccc出现的个数
	// equal_to有两个参数，left来自容器，right:ccc
	// bind2nd函数适配器，把预定义函数对象和第二个参数进行绑定
	// bind2nd在C++11后已经废弃，用bind函数适配器
	// placeholders::_1 是占位符，标定这个是要传入的参数
	// bind()不仅可以用于二元函数，还可以用于多元函数，可以绑定多元函数中的多个参数，不想绑定的参数使用占位符表示
	int num = count_if(v1.begin(), v1.end(), bind(equal_to<string>(), placeholders::_1, "ccc"));
	cout << "num: " << num << endl;
}
class IsGreat {
private:
...
void pre_test2() {
	...
	cout << "equal 3 size: " << num1 << endl;
	//通过预定义的函数对象求大于2的个数
	int num2 = count_if(v1.begin(), v1.end(), bind(greater<int>(), placeholders::_1, 2));
	cout << "greater 2 num: " << num2 << endl;
	//通过谓词求大于2的个数
	int num3 = count_if(v1.begin(), v1.end(), IsGreat(2));
	cout << "greater 2 num: " << num3 << endl;
	//求奇数个数
	int num4 = count_if(v1.begin(), v1.end(), bind(modulus<int>(), placeholders::_1, 2));
	cout << "奇数个数: " << num4 << endl;
	//求偶数个数
	// not1 not2 用not_fn替代
	int num5 = count_if(v1.begin(), v1.end(), not_fn(bind(modulus<int>(), placeholders::_1, 2)));
	cout << "偶数个数: " << num5 << endl;
}
int main() {
	pre_test1();
...
void transform_test1() {
	...
	cout << endl;
	MyShow show = for_each(v1.begin(), v1.end(), MyShow());
	cout << endl;
	show.printNum();
	cout << "------------------------" << endl;
}
...
void transform_test2() {
	...
	list<int> l1;
	l1.resize(v1.size());
	transform(v1.begin(), v1.end(), l1.begin(), bind(multiplies<int>(), placeholders::_1, -10));
	for (auto it = l1.begin(); it != l1.end(); it++) {
		cout << *it << " ";
	}
	cout << endl;
	//transform也可以把运算结果输出到屏幕
	transform(v1.begin(), v1.end(), ostream_iterator<int>(cout, " "), negate<int>());
	cout << endl;
	cout << "-----------------" << endl;
}
//transform所使用的函数对象，参数一般不使用引用，而且还有返回值
int show_element2(int n) {
	int res = n * 2;
	cout << res << " ";
	return res;
}
//transform和for_each比较
void transform_test3() {
	vector<int> v1;
	v1.push_back(1);
	v1.push_back(3);
	v1.push_back(5);
	for_each(v1.begin(), v1.end(), show_element);
	cout << endl;
	transform(v1.begin(), v1.end(), v1.begin(), show_element2);
}
int main() {
	transform_test1();
	transform_test2();
...
void print_v1(vector<int>& v) {
	for (auto it = v.begin(); it != v.end(); it++) {
		cout << *it << " ";
	}
	cout << endl;
}
//adjacent_find在iterator对标识元素范围内，查找一对相邻重复元素，找到则返回指向这对元素的第一个元素的迭代器。
//否则返回past-the-end
void adjacent_find_test() {
	...
	if (it == v1.end()) {
		cout << "v1中没有相邻重复元素" << endl;
	}
	else {
		cout << "v1中相邻重复元素为:" << *it << endl;
		int index = distance(v1.begin(), it);
		cout << *it << " index:" << index << endl;
	}
	print_v1(v1);
}
//binary_search 在有序序列中查找value,找到则返回true.注意：在无序序列中，不可使用。二分查找
void binary_search_test() {
	...
	bool b2 = binary_search(v1.begin(), v1.end(), 4);
	cout << "3 exist: " << b1 << ", 4 exist: " << b2 << endl;
}
...
void count_if_test() {
	...
	cout << "great 3 num is " << num << endl;
}
//find利用底层元素的等于操作符，对指定范围内的元素与输入值进行比较。当匹配时，结束搜索，返回该元素的迭代器
void find_test() {
	vector<int> v1;
	v1.push_back(1);
	v1.push_back(3);
	v1.push_back(3);
	v1.push_back(5);
	auto it1 = find(v1.begin(), v1.end(), 1);
	if (it1 == v1.end()) {
...
void find_if_test() {
	...
	v1.push_back(3);
	v1.push_back(5);
	auto it1 = find_if(v1.begin(), v1.end(), greatThree);
	if (it1 == v1.end()) {
		cout << "not exist > 3 element" << endl;
	}
	else {
		cout << "find > 3: " << *it1 << endl;
	}
}
int main() {
	adjacent_find_test();
...
//sort:以默认升序的方式重写排列指定范围内的元素。若要改变排序规则，可以输入比较函数
class CStudent {
public:
	int id;
	string name;
...
bool Compare(const CStudent& a, const CStudent& b) {
	return a.id < b.id;
}
void sort_test() {
	vector<CStudent> v1;
	v1.push_back(CStudent(1, "A"));
	v1.push_back(CStudent(4, "AAAA"));
	v1.push_back(CStudent(3, "AAA"));
	v1.push_back(CStudent(2, "AA"));
	cout << "排序前" << endl;
	for (auto it = v1.begin(); it != v1.end(); it++) {
...
//replace_if将指定范围内所有操作结果为true的元素用新值替换
bool lessThree(int& element) {
	return element < 3;
}
void replace_if_test() {
	...
	v1.push_back(5);
	v1.push_back(3);
	replace_if(v1.begin(), v1.end(), lessThree, 3);
	for (int i : v1) {
		cout << i << " ";
	}
	cout << endl;
...
void accumulate_test() {
	...
	v1.push_back(3);
	v1.push_back(5);
	int sum = accumulate(v1.begin(), v1.end(), 0); //numeric
	cout << "sum: " << sum << endl;
}
...
void union_test() {
	...
	cout << endl;
	vector<int> v4;
	set_intersection(v1.begin(), v1.end(), v2.begin(), v2.end(), inserter(v4, v4.begin()));
	for (int i : v4) {
		cout << i << " ";
	}
	cout << endl;
