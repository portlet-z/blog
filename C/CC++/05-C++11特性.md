...
int main() {
	test('a', 'b'); //t: a, u: b
	test<int>('a', 'b'); //t: 97, u: b
	test<char>('a', 'b'); //t: a, u: b
	test<int, char>('a', 'b');//t: 97, u: b
	test<char, int>('a', 'b');//t: a, u: 98
	test();//t: 65, u: 66 
	cin.get();
	return 0;
}
...
int main() {
	...
	m2.insert(make_pair(3, 3.3));
	for (auto it = m2.begin(); it != m2.end(); it++) {
		cout << it->first << ":" << it->second << endl;
	}
	//MyMap<string>::mapType m3;
	MMap<string> m3;
	m3.insert(make_pair(1, "aaa"));
	m3.insert(make_pair(2, "bbb"));
	m3.insert(make_pair(3, "ccc"));
...
public:
	...
	Base(int i, double j) : i(i), j(j) {}
	Base(int i, double j, string k) : i(i), j(j), k(k) {}
private:
	int i;
	double j;
	string k;
...
```
# initializer_list
- 使用initializer_list可以使函数的参数接收任意同等类型的元素
```c++
#include <iostream>
