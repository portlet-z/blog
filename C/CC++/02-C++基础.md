...
- protected继承：父类中public,protected成员为protected,父类中private成员仍然为private
...
public:
	Object(int a, int b) {
		this->a = a;
		this->b = b;
		cout << "Object Construtor: a " << a << ", b " << b << endl;
	}
	~Object() {
		cout << "Object Destructor: a " << a << ", b " << b << endl;
	}
};
...
public:
	Parent(char* p) : Object(1, 2) {
		this->p = p;
		cout << "Parent Constructor: " << p << endl;
	}
	~Parent() {
		cout << "Parent Destructor: " << p << endl;
	}
	void print() {
		cout << p << endl;
	}
...
private:
	...
	Object obj2;
public:
	Child(char* c) : Parent(c), obj1(3,4), obj2(5,6) {
		this->c = c;
		cout << "Child Constructor: " << c << endl;
	}
	~Child() {
		cout << "Child Destructor: " << c << endl;
	}
	void print() {
		cout << "c: " << this->c << endl;
	}
...
/*
Object Construtor: a 1, b 2
Parent Constructor: hello
Object Construtor: a 3, b 4
Object Construtor: a 5, b 6
Child Constructor: hello
Child Destructor: hello
Object Destructor: a 5, b 6
Object Destructor: a 3, b 4
Parent Destructor: hello
Object Destructor: a 1, b 2
*/
```
## 继承中的同名成员变量处理方法
- 当子类成员变量与父类成员变量同名时：子类依然从父类继承同名成员
- 在子类中通过作用域分辨符::进行同名成员区分（在派生类中使用基类的同名成员，显示地使用类名限定符）
- 同名成员存储在内存中地不同位置
```c++
#include <iostream>
using namespace std;
...
- 派生类中访问静态成员，用以下形式显示说明： 类名::成员，或通过对象访问 对象名.成员
```c++
#include <iostream>
using namespace std;
...
//这句话不是简单的变量赋值，更重要的时告诉C++编译器要给静态成员a分配内存
int A::a = 100;
...
public:
	D(int a, int b1, int b2) : Base1(b1), Base2(b2), Base(3) {
		this->a = a;
	}
private:
	int a;
...
void how_to_print1(Parent* base) {
	base->print();
}

void how_to_print2(Parent& base) {
	base.print();
}
...
void main() {
	...
	Child c1(20);
	base = &p1;
	base->print(); // Parent a: 10
	base = &c1;
	base->print(); // Parent a: 1

	Parent& p2 = p1;
	p2.print(); // Parent a: 10
	Parent& p3 = c1;
	p3.print(); // Parent a: 1

	how_to_print1(&p1); // Parent a: 10
	how_to_print1(&c1); // Parent a: 1
	 
	how_to_print2(p1); // Parent a: 10
	how_to_print2(c1); // Parent a: 1
	system("pause");
}
```
- 解决方案：使用virtual 声明的函数被重写后即可展现出多态特性
## 多态案例
```c++
#include <iostream>
using namespace std;
...
class AdvHeroFigher : public HeroFighter {
public:
	virtual int power() {
		return 20;
	}
...
void play_obj(HeroFighter* hf, EnemyFigher* ef) {
	//不写virtual关键字时静态联编，C++编译器根据HeroFighter类型，去执行这个类型的power函数，在编译器编译阶段就 已经决定了函数的调用
...
```
- 封装：突破C语言函数的概念：用类做函数参数的时候可以使用对象的属性和对象的方法
- 继承：代码复用
- 多态：可以使用未来
- 实现多态的三个条件：要有继承，要有虚函数重写，用父类指针（引用）指向子类对象
## 虚析构函数
```c++
#include <iostream>
using namespace std;
...
public:
	...
	~C() {
		delete[] p;
		cout << "C Destrutor" << endl;
	}
};

//只执行了父类的析构函数
//向通过父类指针把所有的子类对象的析构函数都执行一遍
//向通过父类指针释放所有的子类资源
void free_obj(A* base) {
	delete base;
}
...
void main() {
	C* c = new C;
	free_obj(c);
	// delete c; //直接通过子类对象释放资源，不需要写virtual
	system("pause");
}
```
## 函数重写&函数重载
- 函数重载：必须在同一个类中进行；子类无法重载父类的函数，父类同名函数将被名称覆盖；重载是在编译期间根据参数类型和个数决定函数调用
- 函数重写：必须发生在父类与子类直接；并且父类与子类中的函数必须有完全相同的原型；使用virtual声明之后能够产生多态（如果不使用virtual，那叫重定义）；多态是在运行期间根据具体对象的类型决定函数调用
```c++
#include <iostream>
using namespace std;
...
public:
	...
	virtual void func(int i, int j, int k) {
		cout << "func() i: " << i << ", j: " << j << ", k: " << k << endl;
	}
};

void main() {
	Child c;
	// c.func(); // err子类无法重载父类的函数，父类同名函数将被名称覆盖
	c.Parent::func();
	system("pause");
}
```
...
- 子类的vptr指针分步完成。当指向父类的构造函数时,c1.vptr指向父类的虚函数表，当父类的构造函数运行完毕后，会把c1.vptr指向子类的虚函数表
```c++
#include <iostream>
using namespace std;
...
public:
	...
	virtual void print() { //子类virtual关键字可写可不写
		cout << "Child" << endl;
	}
};
...
void play(Parent* base) {
	...
	//父类对象和子类对象分别有vptr指针，指向虚函数表，根据虚函数表再找函数的入口的地址
	//迟绑定：运行的时候，C++编译器才去判断
}

void main() {
	Parent p1; //用类定义对象的时候C++编译器会在对象中添加一个vptr指针
	Child c1;  //子类里面也有一个vptr指针
	play(&p1);
	play(&c1);
	cout << sizeof(Parent) << endl; //32bit下8
	cout << sizeof(Child) << endl;  //32bit下12

	Child array[] = { Child(1,1), Child(2,2), Child(3,3) };
	Parent* p = NULL;
	Child* c = NULL;
	p = array;
	c = array;
	...
	c->print();
	p++;
	//p->print(); //指向down掉，父类和子类指针步长不一样
	system("pause");
}
```
# 纯虚函数和抽象类
## 基本概念
- 纯虚函数时在一个基类中说明的虚函数，在基类中没有定义，要求任何派生类都定义自己的版本
- 纯虚函数类似于Java中的抽象方法
...
void figure_play(Figure* f) {
	cout << "area: " << f->get_area() << endl;
}

//Figure func() { //err 抽象类不能做为函数返回类型
//
//}

//void func(Figure f) { //err 抽象类不能作为参数类型
//
//}

Figure& func(Figure& f) { //ok 抽象类的引用可以作为返回类型和函数参数
	return f;
}

void main() {
	// Figure f;// err 抽象类不能被实例化
	Figure* f = NULL;// 可以定义抽象类的指针
...
- 绝大多数面向对象的语言都不支持多继承；绝大多数的面向对象的语言都有接口的概念；C++中没有接口的概念；C++中可以使用纯虚函数实现接口
...
int MainOp0(MyTypeFuncAdd myTypeFuncAdd) {
	return myTypeFuncAdd(5, 6);
}

int MainOp1(int (*MyTypeFuncAdd)(int a, int b)) {
	return MyTypeFuncAdd(5, 6);
}

void main() {
	MyTypeFuncAdd myTypeFuncAdd = add;
	cout << myTypeFuncAdd(3, 4) << endl; //间接调用

	cout << MainOp0(add) << endl;
	cout << MainOp1(add) << endl;

	cout << MainOp0(add2) << endl;
	cout << MainOp1(add2) << endl;
