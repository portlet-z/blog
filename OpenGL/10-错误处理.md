- 我们由两种主要的方法来检查在OpenGL中的错误，其中一个叫glGetError,它是一个我们可以调用的函数，它一开始就一直在OpenGL中，它兼容所有版本。每次我们调用OpenGL函数，如果发生错误，在OpenGL内存中，有一个标志会被内部设置
- 当我们调用glGetError时，它会返回一个标志
- 在OpenGL4.3中，OpenGL增加了一个函数，glDebugMessageCallback ,它允许我们指定一个指向OpenGL的函数指针。OpenGL会调用我们那个函数，当错误发生时。这比glGetError好多了，因为不是我们不断地问OpenGL是否有错误而是当出现问题时，它会自己告诉我们。它实际上会给你更详细地信息而不是只给你一个错误码

- glGetError当没有错误时会返回GL_NO_ERROR，我们应该在循环中调用它，以确保我们得到所有地错误
