- 在应用程序启动后，操作系统要做的就是会将整个程序加载到内存，并分配一大堆物理ram以便我们的实际应用程序可以允许。堆和栈是ram中实际存在的两个区域，栈通常是一个预定义大小的内存区域，通常约为2M左右。堆也是一个预定义了默认值的区域，但是它可以增长，并随着应用程序的进行而改变。重要的是，要知道这两个内存区域的实际位置（物理位置），在我们的ram中是完全一样的
